...
public class Car_Version_Colors extends Fragment implements View.OnClickListener {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        if (Car_Version_Overview.Available_Colours != null) {
            try {

                if (Car_Version_Overview.Available_Colours.length() > 0) {
                    for (int i = 0; i < Car_Version_Overview.Available_Colours.length(); i++) {
                        final JSONObject jsonObject = Car_Version_Overview.Available_Colours.getJSONObject(i);
...
//                        linearLayoutParent.addView(customTextViewDes);
    ...
    public void onClick(View v) {
        if (Car_Version_Overview.Available_Colours != null) {
            try {
                Toast.makeText(activity, Car_Version_Overview.Available_Colours.getJSONObject((int) v.getTag()).getString("colourname"), Toast.LENGTH_SHORT).show();
